"""
Baseline 1- 2 KoClip Model Evaluation Script

This script uses koCLIP (Bingsu/clip-vit-large-patch14-ko) to predict and evaluate 
    ...
    for i, row in tqdm(df_subset.iterrows(), total=len(df_subset), desc="Computing similarities"):
        try:
            ...
            img_embed, txt_embed = get_clip_embeddings(
                image_path, row['Hashtag'], model, processor, device
            )
            ...
            if (i + 1) % 100 == 0:
                print(f"[{i+1}] Score: {row['Score']} | Similarity: {sim:.4f} | Hashtag: {row['Hashtag'][:50]}...")
...
def save_embeddings(train_sims, train_scores, val_sims, val_scores, output_dir="./results/"):
    ...
    np.save(os.path.join(output_dir, "train_scores_base1_2.npy"), np.array(train_scores))
    np.save(os.path.join(output_dir, "val_similarities_base1_2.npy"), np.array(val_sims))
    np.save(os.path.join(output_dir, "val_scores_base1_2.npy"), np.array(val_scores))
