...
# -*- coding: utf-8 -*-
"""
Baseline 2-1 CLIP Fine-tuning Model Evaluation Script

This script fine-tunes the CLIP model on an Instagram dataset of image–hashtag pairs
to classify and evaluate the semantic consistency between images and hashtags.

Key points:
- Dataset: ~2,000 samples collected via Python-based Selenium crawling and labeled using
  GPT-4 Structured Output (5-level scale from Very Low to Very High consistency).
- Model: Fine-tuned OpenAI CLIP (clip-vit-base-patch32) with an added classifier head
  to predict 5 consistency classes.
- Evaluation: Both an 8:2 train/validation split and Stratified K-Fold cross-validation
  are used to measure model performance.
- Metrics: Validation Accuracy, Weighted F1 Score, and Confusion Matrices are reported.

This script serves as a baseline to identify the limitations of CLIP on Korean
Instagram data and provides a foundation for comparison with koCLIP and other
Korean-specialized multimodal models.
...
class FrozenCLIPWithClassifier(nn.Module):
    """CLIP with frozen encoder and trainable classifier head"""
    def __init__(self, clip_model_name="openai/clip-vit-base-patch32", num_classes=5):
        super(FrozenCLIPWithClassifier, self).__init__()
...
def main():
    ...
    try:
        ...
        comparison_path = os.path.join(output_dir, "baseline2_1_method_comparison.csv")
